...
/**
 * Migration script to transfer Marx works from old data structure to new folder-based structure
 * 
 * Old structure: data/marx-works-by-subject.json (flat array with subject field)
 * New structure: public/data-v2/first-international/Karl Marx/{Subject}.json (separate files per subject)
 */
...
const OLD_FILE = path.join(__dirname, '../../data/marx-works-by-subject.json');
const NEW_DIR = path.join(__dirname, '../../public/data-v2/first-international/Karl Marx');

function main() {
  console.log('Starting Marx works migration...\n');
  
  // Read old data
  console.log(`Reading old data from: ${OLD_FILE}`);
  const oldDataRaw = fs.readFileSync(OLD_FILE, 'utf-8');
  const oldWorks = JSON.parse(oldDataRaw);
  console.log(`Found ${oldWorks.length} works in old structure\n`);
  ...
  for (const work of oldWorks) {
    ...
    if (!worksBySubject.has(subject)) {
      worksBySubject.set(subject, []);
    }
    worksBySubject.get(subject).push(newWork);
  }
  ...
  console.log(`  Total works migrated: ${oldWorks.length}`);
